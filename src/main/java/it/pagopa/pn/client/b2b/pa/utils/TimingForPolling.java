...
public class TimingForPolling {
    ...
    public TimingResult getTimingForStatusValidation(String element) {
        ...
        if (element.equalsIgnoreCase(PnB2bClientTimingConfigs.DefaultsElementTimingValue.ACCEPTED_SHORT_VALIDATION.toString())) {
            waiting = timingConfigs.getWaitMillisShort();
        } else if (element.equalsIgnoreCase(PnB2bClientTimingConfigs.DefaultsElementTimingValue.ACCEPTED_EXTRA_RAPID_VALIDATION.toString())) {
