...
import static it.pagopa.pn.client.b2b.pa.domain.Costanti.STATUS_EXTRA_RAPID;
import static it.pagopa.pn.client.b2b.pa.domain.Costanti.STATUS_RAPID;
import static it.pagopa.pn.client.b2b.pa.domain.Costanti.STATUS_SLOW;
import static it.pagopa.pn.client.b2b.pa.domain.Costanti.TIMELINE_EXTRA_RAPID;
...
import static it.pagopa.pn.client.b2b.pa.domain.Costanti.TIMELINE_SLOW_E2E;
...
import static it.pagopa.pn.client.b2b.pa.domain.Costanti.WEBHOOK;
...
public class NotificationUtilsV26 extends B2bUtils {


    @Autowired
    public NotificationUtilsV26(ApplicationContext context, IPnPaB2bClient b2bClient, PnPollingFactory pollingFactory) {
        super(context, b2bClient, pollingFactory);
    }

    public synchronized NewNotificationRequestV26 convertNotificationRequest(Map<String, String> data) {
        NewNotificationRequestV26 notificationRequest = (new NewNotificationRequestV26()
                ...
                .pagoPaIntMode((getValue(data, PAGOPAINTMODE.key).equalsIgnoreCase("SYNC") ?
                        NewNotificationRequestV26.PagoPaIntModeEnum.SYNC :
                        (getValue(data, PAGOPAINTMODE.key).equalsIgnoreCase("ASYNC") ?
                                NewNotificationRequestV26.PagoPaIntModeEnum.ASYNC :
                                getValue(data, PAGOPAINTMODE.key).equalsIgnoreCase("NONE") ?
                                        NewNotificationRequestV26.PagoPaIntModeEnum.NONE : null))));
    ...
    private NewNotificationRequestV26 addDocument(NewNotificationRequestV26 notificationRequest, Map<String, String> data) {
    ...
    //payDocument
    private void preloadPayDocument(NewNotificationRequestV26 request) throws IOException {
    ...
    public void verifyNotification(String iun) throws IllegalStateException {
        FullSentNotificationV29 fsn = b2bClient.getSentNotificationV29(iun);
        //Verify Sha
    ...
    private void verifySha256Notification(FullSentNotificationV29 fsn) {
        for (NotificationDocument doc : fsn.getDocuments()) {
            int docIdx = Integer.parseInt(Objects.requireNonNull(doc.getDocIdx()));
    ...
    private void extractAndCheckAttachment(FullSentNotificationV29 fsn, NotificationRecipientV24 recipient) {
    ...
    private void extractAttachment(FullSentNotificationV29 fsn, NotificationRecipientV24 recipient) {
    ...
    public NewNotificationResponse uploadNotification(NewNotificationRequestV26 request, String errorType) throws IOException {
        ...
        return b2bClient.sendNewNotificationV26(request);
    }
    ...
    private NewNotificationRequestV26 addErrorsOnPurpose(NewNotificationRequestV26 request, String errorType) throws IOException {
        if (errorType != null) {
            switch (errorType) {
