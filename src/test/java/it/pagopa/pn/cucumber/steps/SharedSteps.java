    ...
    public void vengonoInviateNotifichePerLUtenteSignorCasualeConIlESiAspettaFinoAlloStatoCOMPLETELY_UNREACHABLE(int numberOfNotification, String pa) {
        ...
        for (NewNotificationRequestV23 notification : notificationRequests) {
            Thread t = new Thread(() -> {
                ...
                threadWait(getWait());
                FullSentNotificationV24 fullSentNotificationV24 = b2bUtils.waitForRequestAcceptation(internalNotificationResponse);
                ...
                for (int i = 0; i < 33; i++) {
                    ...
                    fullSentNotificationV24 = b2bClient.getSentNotification(fullSentNotificationV24.getIun());
                    log.info("NOTIFICATION_TIMELINE: " + fullSentNotificationV24.getTimeline());
                    timelineElement = fullSentNotificationV24.getTimeline().stream().filter(elem -> Objects.requireNonNull(elem.getCategory()).equals(TimelineElementCategoryV23.COMPLETELY_UNREACHABLE)).findAny().orElse(null);
                    if (timelineElement != null) {
                        break;
                    }
                }
                Assertions.assertNotNull(timelineElement);
                sentNotifications.add(fullSentNotificationV24);
            });
        ...
        log.debug("IUN: ");
        for (FullSentNotificationV24 notificationV24 : sentNotifications) {
            log.info(notificationV24.getIun());
        }
