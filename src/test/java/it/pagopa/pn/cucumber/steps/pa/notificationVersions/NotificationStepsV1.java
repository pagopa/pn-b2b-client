    ...
    public void sendNotification(String status, int wait) {
        try {
            Assertions.assertDoesNotThrow(() -> {
                ...
                notificationResponse = sharedSteps.getB2bUtils().uploadNotificationV1(notificationRequest);
                threadWait(wait);
                fullSentNotification = sharedSteps.getB2bUtils().waitForRequestAcceptationV1(notificationResponse);
            });
            threadWait(wait);
