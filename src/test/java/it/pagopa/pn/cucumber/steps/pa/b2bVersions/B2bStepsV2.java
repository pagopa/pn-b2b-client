...
public class B2bStepsV2 implements B2bStepsInterface {
    ...
    public void checkFullSentNotificationWithVersion(boolean isPresent, String timelineEventCategory) {
        ...
        FullSentNotificationV20 fullSentNotification = b2bSteps.getB2bClient().getSentNotificationV2(b2bSteps.getSharedSteps().getNotificationIun());
    ...
    public void readEventsUpToTimelineElement(String timelineEventCategory) {
        PnPollingServiceTimelineSlowV20 timelineSlow =
                (PnPollingServiceTimelineSlowV20) b2bSteps.getSharedSteps().getPollingFactory().getPollingService(PnPollingStrategy.TIMELINE_SLOW_V20);
        PnPollingResponseV20 pnPollingResponse = timelineSlow.waitForEvent(
    ...
    public void readEventsUpToStatus(String status) {
        ...
        PnPollingServiceStatusRapidV20 statusRapid = (PnPollingServiceStatusRapidV20) b2bSteps.getSharedSteps().getPollingFactory().getPollingService(PnPollingStrategy.STATUS_RAPID_V20);
        PnPollingResponseV20 pnPollingResponse = statusRapid.waitForEvent(iun,
                PnPollingParameter.builder()
                        .value(status)
                        .build());

        fullSentNotification = pnPollingResponse.getNotification();
        log.info("NOTIFICATION_STATUS_HISTORY V20: " + pnPollingResponse.getNotification().getNotificationStatusHistory());
        try {
            assertThat(pnPollingResponse.getResult())
            ...
            log.info("NOTIFICATION_STATUS_HISTORY_ELEMENT V20: " + notificationStatusHistoryElement);
    ...
    public void checkPriceForRecipient(int recipientIndex, String price) {
        ...
        NotificationPriceResponse notificationPrice =
                b2bSteps.getB2bClient().getNotificationPrice(fullSentNotification.getRecipients().get(
                        recipientIndex).getPayment().getCreditorTaxId(), fullSentNotification.getRecipients().get(recipientIndex).getPayment().getNoticeCode());
