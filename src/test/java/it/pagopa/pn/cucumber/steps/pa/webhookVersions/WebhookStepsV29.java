...
public class WebhookStepsV29 implements WebhookStepsInterface {
    ...
    public Object searchStatusElementInWebhook(String lastEventId, int deepCount, int position, AvanzamentoNotificheWebhookB2bSteps.StatusElementSearchResult<?> statusForStream) {
        NotificationStatusV28 status = ((NotificationStatusV28) statusForStream.getNotificationStatus());
    ...
    public boolean checkStatus(AvanzamentoNotificheWebhookB2bSteps.StatusElementSearchResult<?> statusForStream) {
        NotificationStatusV28 notificationInternalStatus = NotificationStatusV28.valueOf(((NotificationStatusV28) statusForStream.getNotificationStatus()).name());
        boolean found = false;
        for (int i = 0; i < statusForStream.getNumCheck(); i++) {
            ...
            FullSentNotificationV28 fullSentNotification = getFullSentNotificationVersioned();
            NotificationStatusHistoryElementV28 notificationStatusHistoryElement = fullSentNotification.getNotificationStatusHistory().stream().filter(
    ...
    public <T> AvanzamentoNotificheWebhookB2bSteps.StatusElementSearchResult<T> getStatusEventForStream(String notificationStatusName, TimingForPolling.TimingResult timingForElement) {
        AvanzamentoNotificheWebhookB2bSteps.StatusElementSearchResult<NotificationStatusV28> result = new AvanzamentoNotificheWebhookB2bSteps.StatusElementSearchResult<>();
        result.setNotificationStatus(NotificationStatusV28.valueOf(notificationStatusName));
        result.setWaiting(timingForElement.waiting());
        result.setNumCheck(timingForElement.numCheck());
    ...
    public <T> PnPollingWebhook getPnPollingWebhook(T timeLineOrStatus) {
        ...
        } else if (timeLineOrStatus instanceof NotificationStatusV28) {
            pnPollingWebhook.setNotificationStatusV29((NotificationStatusV28) timeLineOrStatus);
            progressResponseElementList.clear();
            pnPollingWebhook.setProgressResponseElementListV29(progressResponseElementList);
        }
    ...
    public void verificaDeanonimizzazioneEventiTimeline(boolean isDigitale, boolean withDelega) {
        ...
        TimelineElementDetailsV28 timelineElementWebhookDetails = progressResponseElement.getElement().getDetails();
    ...
    public void checkLegalFactCategory(String timelineCategory, String legalFactCategory, boolean arePresent) {
        ...
        assertThat(timelineElementWithTargetCategory)
                ...
                .isNotNull();
        List<it.pagopa.pn.client.b2b.pa.generated.openapi.clients.externalb2bpa.model.LegalFactsIdV28> elementsWithLegalFactCategory =
