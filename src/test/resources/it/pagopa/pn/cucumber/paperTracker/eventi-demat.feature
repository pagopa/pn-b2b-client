...
Feature: Casi di test relativi alla nuova gestione degli eventi di dematerializzazione introdotta in paper-tracker
  ...
  Scenario Outline: [EVENTI_DEMAT_890_1] Il sistema riceve un evento di dematerializzazione per il prodotto 890 dopo l'evento finale (C o F)
  e il nuovo evento di dematerializzazione è visibile in timeline
    Given viene generata una nuova notifica
      | subject               | invio notifica con cucumber |
      | senderDenomination    | Comune di Palermo           |
    ...
    Examples:
      ...
      | Via@FAIL-Irreperibile_890         | RECAG003E                  | Indagine     |
  ...
  Scenario Outline: [EVENTI_DEMAT_RS_1] Il sistema riceve un evento di dematerializzazione per il prodotto RS dopo l'evento finale (C o F)
  e il nuovo evento di dematerializzazione è visibile in timeline
    Given viene generata una nuova notifica
      | subject            | invio notifica con cucumber |
      | senderDenomination | Comune di Palermo           |
    ...
    Examples:
      ...
      | Via@FAIL_RS_IRREPERIBILE_ASSOLUTO      | RECRS002E                  |
  ...
  e il nuovo evento di dematerializzazione è visibile in timeline
    ...
    Then viene invocato il consolidatore con i seguenti dati:
      | productType | attemptId | pcRetry   | recIndex   | statusCode | deliveryFailureCause | attachment_1 | attachment_2 |
      | RIR         | ATTEMPT_0 | PCRETRY_0 | RECINDEX_0 | RECRSI004B |                      | Indagine     |              |
    And viene verificato che l'elemento di timeline "SEND_ANALOG_PROGRESS" esista
      | loadTimeline               | true                           |
      | details                    | NOT_NULL                       |
