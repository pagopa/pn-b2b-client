...
Feature: Polling dello Stato del Messaggio ed Evoluzione Temporale degli Snapshot
  ...
  Scenario Outline: [MOCK_IO_POLLING_03_1] Evoluzione temporale dello snapshot del messaggio (<finestra_temporale>)
    ...
    Examples:
      ...
      | T0_ELAPSED_LESS_30S   | ACCEPTED  | NON_DISPONIBILE | NON_DISPONIBILE |
      | T1_ELAPSED_30_60S     | PROCESSED | UNREAD          | NOT_PAID        |
      | T2_ELAPSED_60_90S     | PROCESSED | READ            | NOT_PAID        |
      | T3_ELAPSED_OVER_90S   | PROCESSED | READ            | PAID            |

  @MOCK_IO_POLLING_03_2_A
  Scenario: [MOCK_IO_POLLING_03_2_A] Rifiuto polling per timestamp non numerico o id malformato
    Given una richiesta di stato messaggio con identificativo mock non numerico "MOCK-OK_READ-notanumber-rand1234"
    When viene richiesto lo stato del messaggio per il destinatario "RSSMRA80A01H5010"
    Then la richiesta di stato messaggio viene rifiutata per identificativo mock non valido

  @MOCK_IO_POLLING_03_2_B
  Scenario Outline: [MOCK_IO_POLLING_03_2_B] Rifiuto polling per sequenza non censita
