Feature: calcolo costo notifica in base hai grammi con notifiche RS
  ...
  Scenario Outline: [CALCOLO-COSTO_RS-20GR_1] (Gennaio) Invio notifica e verifica calcolo del costo su raccomandata con peso <= 20gr
    ...
    Examples:
      ...
      | 27062 | 398   | CAMPOSPINOSO ALBAREDO   | PV       | notifica analogica RECAPITISTA |
      | 92038 | 342   | MONTEVAGO               | AG       | notifica analogica RECAPITISTA |
      | 92044 | 342   | PALMA DI MONTECHIARO    | AG       | notifica analogica RECAPITISTA |

      | 14027 | 398   | TONENGO              | AT       | notifica analogica RECAPITISTA |

  @costoAnalogicoGennaio25
  Scenario: [CALCOLO-COSTO_RS-20GR_2] (Gennaio) Invio notifica ZONE_1 e verifica calcolo del costo su raccomandata con peso <= 20gr
    Given viene generata una nuova notifica
  ...
  @costoAnalogicoGennaio25
  Scenario: [CALCOLO-COSTO_RS-20GR_4] (Gennaio) Invio notifica ZONE_3 e verifica calcolo del costo su raccomandata con peso <= 20gr
    Given viene generata una nuova notifica
      | subject            | notifica analogica con cucumber |
      | senderDenomination | Comune di palermo               |
      | feePolicy          | DELIVERY_MODE                   |
