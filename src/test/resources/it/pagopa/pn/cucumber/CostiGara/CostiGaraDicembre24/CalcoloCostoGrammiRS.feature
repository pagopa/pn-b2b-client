Feature: calcolo costo notifica in base hai grammi con notifiche RS
  ...
  Scenario Outline: [CALCOLO-COSTO_RS-20GR_1] (Dicembre) Invio notifica e verifica calcolo del costo su raccomandata con peso <= 20gr
  ...
  @costoAnalogicoDicembre24
  Scenario: [CALCOLO-COSTO_RS-20GR_3] (Dicembre) Invio notifica ZONE_2 e verifica calcolo del costo su raccomandata con peso <= 20gr
    Given viene generata una nuova notifica
      | subject            | notifica analogica con cucumber |
      | senderDenomination | Comune di palermo               |
      | feePolicy          | DELIVERY_MODE                   |
  ...
  @costoAnalogicoDicembre24
  Scenario: [CALCOLO-COSTO_RS-20GR_4] (Dicembre) Invio notifica ZONE_3 e verifica calcolo del costo su raccomandata con peso <= 20gr
    Given viene generata una nuova notifica
      | subject            | notifica analogica con cucumber |
      | senderDenomination | Comune di palermo               |
      | feePolicy          | DELIVERY_MODE                   |
