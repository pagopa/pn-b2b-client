Feature: test per la visualizzazione della copia conforme
  ...
  Scenario: [COPIA-CONFORME-CON020-3] Creazione notifica RS analogica mono destinatario verso PF con SEND_SIMPLE_REGISTERED_LETTER_PROGRESS per PA e PF
    ...
    And si crea il nuovo stream per il "Comune_Multi" con versione "V24"
    When la notifica viene inviata tramite api b2b dal "Comune_Multi" e si attende che lo stato diventi ACCEPTED
    Then vengono letti gli eventi dello stream del "Comune_Multi" fino all'elemento di timeline "SEND_SIMPLE_REGISTERED_LETTER_PROGRESS" con la versione V24 con deliveryDetailCode "CON020"
  ...
  @copiaConformeCon020
  Scenario: [COPIA-CONFORME-CON020-TEST-MULTI] Creazione notifica AR analogica da ZIP multi destinatario verso PG con Pec
    Given viene generata una nuova notifica
      | subject            | invio notifica con cucumber |
      | senderDenomination | Comune di Palermo           |
