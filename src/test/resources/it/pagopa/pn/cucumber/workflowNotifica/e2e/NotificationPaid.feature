Feature: Il pagamento esterno non sostituisce la consegna della notifica
  ...
  Scenario Outline: [E2E-WF-POST-PAYMENT-1.2] Il pagamento non blocca il secondo tentativo analogico <prodotto>
    ...
    And si attende il corretto pagamento della notifica
    And vengono letti gli eventi fino all'elemento di timeline della notifica "PAYMENT"
    And l'evento "PAYMENT" è successivo all'evento "SEND_ANALOG_DOMICILE"
  ...
  Scenario: [E2E-WF-POST-PAYMENT-1.3] Il pagamento non blocca la raccomandata semplice dopo il fallimento digitale
    ...
    And si attende il corretto pagamento della notifica
    And vengono letti gli eventi fino all'elemento di timeline della notifica "PAYMENT"
    And vengono letti gli eventi fino all'elemento di timeline della notifica "SEND_DIGITAL_FEEDBACK" al tentativo "ATTEMPT_1"
    And vengono letti gli eventi fino all'elemento di timeline della notifica "DIGITAL_DELIVERY_CREATION_REQUEST"
  ...
  Scenario Outline: [E2E-WF-POST-PAYMENT-2.1] La visualizzazione blocca il primo invio analogico <prodotto> anche con notifica pagata
    Given viene generata una nuova notifica
  ...
  @e2e @postPaymentWorkflow @regression
  Scenario Outline: [E2E-WF-POST-PAYMENT-2.2] La visualizzazione blocca il secondo tentativo analogico <prodotto> anche con notifica pagata
    Given viene generata una nuova notifica
    ...
    And viene verificato che l'elemento di timeline "SEND_ANALOG_FEEDBACK" esista
      ...
      | details_sentAttemptMade | 0        |
      | details_responseStatus  | KO       |
    And l'evento "SEND_ANALOG_FEEDBACK" è successivo all'evento "NOTIFICATION_VIEWED"
  ...
  @e2e @postPaymentWorkflow @regression
  Scenario: [E2E-WF-POST-PAYMENT-2.3] La visualizzazione blocca la raccomandata semplice anche con notifica pagata
    Given viene generata una nuova notifica
    ...
    And destinatario
      ...
      | payment_f24             | NULL               |
    When la notifica viene inviata tramite api b2b dal "Comune_Multi" e si attende che lo stato diventi "ACCEPTED"
    And vengono letti gli eventi fino all'elemento di timeline della notifica "SEND_DIGITAL_FEEDBACK" con responseStatus "KO"
    And l'avviso pagopa viene pagato correttamente
    And la notifica può essere correttamente recuperata da "Cristoforo Colombo"
    And vengono letti gli eventi fino all'elemento di timeline della notifica "NOTIFICATION_VIEWED"
    And si attende il corretto pagamento della notifica
  ...
  @e2e @postPaymentWorkflow @regression
  Scenario Outline: [E2E-WF-POST-PAYMENT-3.1] L'annullamento blocca il primo invio analogico <prodotto> anche con notifica pagata
    Given viene generata una nuova notifica
  ...
  @e2e @postPaymentWorkflow @regression
  Scenario Outline: [E2E-WF-POST-PAYMENT-3.2] L'annullamento blocca il secondo tentativo analogico <prodotto> anche con notifica pagata
    Given viene generata una nuova notifica
    ...
    And viene verificato che l'elemento di timeline "SEND_ANALOG_FEEDBACK" esista
      ...
      | details_sentAttemptMade | 0        |
      | details_responseStatus  | KO       |
    And l'evento "SEND_ANALOG_FEEDBACK" è successivo all'evento "NOTIFICATION_CANCELLATION_REQUEST"
    Then viene verificato che non esista l'elemento "PREPARE_ANALOG_DOMICILE" al tentativo "ATTEMPT_1"
    And viene verificato che il numero di elementi di timeline "SEND_ANALOG_DOMICILE" della notifica sia di 1
  ...
  @e2e @postPaymentWorkflow @regression
  Scenario: [E2E-WF-POST-PAYMENT-3.3] L'annullamento blocca la raccomandata semplice anche con notifica pagata
    Given viene generata una nuova notifica
