Feature: tentativo consumo stream
  ...
  Scenario: [B2B-STREAM_ES1.3_126] Consumo di uno stream notifica con gruppo, con eventType "STATUS"  utilizzando un apikey con gruppo diverso.
    ...
    And viene impostata l'apikey appena generata
    When vengono letti gli eventi dello stream versione "V28"
  ...
  Scenario: [B2B-STREAM_ES1.5_139] Creazione di uno stream senza gruppo con la V28 e lettura Eventi di timeline o di cambio di stato con la versione V10  utilizzando un apikey abilitata
    ...
    And viene aggiornata la apiKey utilizzata per gli stream
    And si crea il nuovo stream per il "Comune_Multi" con versione "V28"
    When vengono letti gli eventi di timeline dello stream con versione "V10" nonostante sia stato creato con la "V28" - Cross Versioning
  ...
  Scenario: [B2B-STREAM_ES1.5_141] Creazione di uno stream senza gruppo con la V10 e  lettura Eventi di timeline o di cambio di stato con la versione V28 utilizzando un apikey abilitata.
    ...
    And Viene creata una nuova apiKey per il comune "Comune_Multi" senza gruppo
    And viene impostata l'apikey appena generata
    When vengono letti gli eventi di timeline dello stream con versione "V28" nonostante sia stato creato con la "V10" - Cross Versioning
    Then l'operazione ha prodotto un errore con status code "403"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
