Feature: tentativo consumo stream
  ...
  Scenario: [B2B-STREAM_ES1.3_126] Consumo di uno stream notifica con gruppo, con eventType "STATUS"  utilizzando un apikey con gruppo diverso.
    ...
    And viene impostata l'apikey appena generata
    When vengono letti gli eventi dello stream versione "V25"
  ...
  @webhookV25 @precondition @cleanWebhook @webhook2
  Scenario: [B2B-STREAM_ES1.5_139] Creazione di uno stream senza gruppo con la V25 e lettura Eventi di timeline o di cambio di stato con la versione V10  utilizzando un apikey abilitata
    Given si predispone 1 nuovo stream denominato "stream-test" con eventType "TIMELINE" con versione "V25"
    And Viene creata una nuova apiKey per il comune "Comune_Multi" senza gruppo
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    And si crea il nuovo stream per il "Comune_Multi" con versione "V25"
    When vengono letti gli eventi di timeline dello stream con versione "V10" nonostante sia stato creato con la "V25" -Cross Versioning
    Then l'operazione ha prodotto un errore con status code "400"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata

  @webhookV25 @precondition @cleanWebhook @webhook2
  Scenario: [B2B-STREAM_ES1.5_141] Creazione di uno stream senza gruppo con la V10 e  lettura Eventi di timeline o di cambio di stato con la versione V25 utilizzando un apikey abilitata.
    ...
    And Viene creata una nuova apiKey per il comune "Comune_Multi" senza gruppo
    And viene impostata l'apikey appena generata
    When vengono letti gli eventi di timeline dello stream con versione "V25" nonostante sia stato creato con la "V10" -Cross Versioning
    Then l'operazione ha prodotto un errore con status code "403"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
