Feature: replace streamID webhook
  ...
  Scenario: [B2B-STREAM_ES1.1_101] Creazione con replaceID di uno stream notifica con gruppo uguale al precedente stream con eventType "STATUS" utilizzando un apikey master. (replacedStreamId settato).
    ...
    And viene aggiornata la apiKey utilizzata per gli stream
    When si crea il nuovo stream con versione "V27" per il "Comune_Multi" con replaceId con un gruppo disponibile "FIRST"
  ...
  Scenario: [B2B-STREAM_ES1.1_103]  Creazione con replaceID di uno stream notifica con gruppo  non appartenente alla PA, con eventType "STATUS" utilizzando un apikey master.(replacedStreamId settato).
    ...
    And si crea il nuovo stream con versione "V27" per il "Comune_Multi" con un gruppo disponibile "FIRST"
    And lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V27"
    When si crea il nuovo stream con versione "V27" per il "Comune_Multi" con replaceId con un gruppo disponibile "ALTRA_PA"
  ...
  Scenario: [B2B-STREAM_ES1.1_104]  Creazione con replaceID di uno stream notifica con gruppo diverso al precedente stream con eventType "STATUS" utilizzando un apikey master. (replacedStreamId settato).
    ...
    And si crea il nuovo stream con versione "V27" per il "Comune_Multi" con un gruppo disponibile "FIRST"
    And lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V27"
    When si crea il nuovo stream con versione "V27" per il "Comune_Multi" con replaceId con un gruppo disponibile "LAST"
  ...
  Scenario: [B2B-STREAM_ES1.1_111]  Creazione con replaceID di uno stream notifica senza gruppo uguale al precedente stream con eventType "STATUS" utilizzando un apikey master. (replacedStreamId settato).
    ...
    And lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V27"
    When si crea il nuovo stream con versione "V27" per il "Comune_Multi" con replaceId con un gruppo disponibile "NO_GROUPS"
    Then lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V27"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  Scenario: [B2B-STREAM_ES1.1_113]  Creazione con replaceID di uno stream notifica senza gruppo con uno stream con gruppo con eventType "STATUS" utilizzando un apikey con gruppo. (replacedStreamId settato)..
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    And si crea il nuovo stream con versione "V27" per il "Comune_Multi" con replaceId con un gruppo disponibile "FIRST"
    When lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V27"
    Then l'operazione ha prodotto un errore con status code "403"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
...
#CONTROLLARE IL CORRETTO COMPORTAMENTO
  ...
  Scenario: [B2B-STREAM_ES1.1_117]  Creazione per una PA di 10 nuovi stream notifica con eventType TIMELINE e senza gruppo, disabilitazione di uno stream e creazione di un nuovo stream.
    ...
    And lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V27"
    And si disabilita lo stream creato per il comune "Comune_Multi" con versione "V27" e apiKey aggiornata
  ...
  Scenario: [B2B-STREAM_ES1.1_102]  Creazione con replaceID di uno stream notifica senza gruppo uguale al precedente stream disabilitato con eventType "TIMELINE" utilizzando un apikey master. (replacedStreamId settato).
    ...
    And si crea il nuovo stream con versione "V27" per il "Comune_Multi" con un gruppo disponibile "NO_GROUPS"
    And lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V27"
    And si disabilita lo stream "V27" creato per il comune "Comune_Multi"
    And l'operazione non ha prodotto errori
    When si crea il nuovo stream con versione "V27" per il "Comune_Multi" con replaceId con un gruppo disponibile "NO_GROUPS"
    Then l'operazione ha prodotto un errore con status code "403"
    And viene modificato lo stato dell'apiKey in "BLOCK"
  ...
  @webhookV27 @precondition @cleanWebhook @webhook2
  Scenario: [B2B-STREAM_ES1.5_142] Creazione di uno stream notifica con gruppo, con eventType "TIMELINE" con V27 e repleceId di uno stream V10 senza gruppo e utilizzando un apikey con gruppo con la versione V27.(replacedStreamId di uno stream creato con la versione V10 settato).
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    Then si crea il nuovo stream con versione "V10_V27" per il "Comune_Multi" con replaceId con un gruppo disponibile "NO_GROUPS"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata