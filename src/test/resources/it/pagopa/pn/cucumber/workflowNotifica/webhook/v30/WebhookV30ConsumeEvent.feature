Feature: avanzamento notifiche webhook b2b V30

  #COMUNE 1
  @webhookV30 @precondition @cleanWebhook @webhook1
  Scenario: [B2B-STREAM_ES1.1_112] Creazione con replaceID di uno stream notifica senza gruppo uguale al precedente stream con eventType "TIMELINE" utilizzando un apikey master. (replacedStreamId settato) con controllo EventId incrementale e senza duplicati.
    ...
    And si crea il nuovo stream con versione "V30" per il "Comune_1" con un gruppo disponibile "NO_GROUPS"
    ...
    Then vengono letti gli eventi dello stream del "Comune_1" fino allo stato "ACCEPTED" con la versione "V30"
    #TEST LETTURA REQUEST_ACCEPTED
    And vengono letti gli eventi dello stream del "Comune_1" fino all'elemento di timeline "REQUEST_ACCEPTED" con la versione "V30"
    #TEST LETTURA DIGITAL_SUCCESS_WORKFLOW
    Then vengono letti gli eventi dello stream del "Comune_1" fino all'elemento di timeline "DIGITAL_SUCCESS_WORKFLOW" con la versione "V30"
    And viene verificato che il ProgressResponseElement del webhook abbia un EventId incrementale e senza duplicati "V30"
    ...
    And vengono letti gli eventi dello stream del "Comune_1" fino all'elemento di timeline "SEND_COURTESY_MESSAGE" con la versione "V30" e apiKey aggiornata con position 0
    And verifica corrispondenza tra i detail del webhook e quelli della timeline con la versione "V30"
    And vengono letti gli eventi dello stream del "Comune_1" fino all'elemento di timeline "SEND_DIGITAL_DOMICILE" con la versione "V30" e apiKey aggiornata con position 0
    And verifica corrispondenza tra i detail del webhook e quelli della timeline con la versione "V30"
    And vengono letti gli eventi dello stream del "Comune_1" fino all'elemento di timeline "SEND_DIGITAL_FEEDBACK" con la versione "V30" e apiKey aggiornata con position 0
    ...
    And verifica corrispondenza tra i detail del webhook e quelli della timeline con la versione "V30"
    And vengono letti gli eventi dello stream del "Comune_1" fino all'elemento di timeline "DIGITAL_SUCCESS_WORKFLOW" con la versione "V30" e apiKey aggiornata con position 0
    And verifica corrispondenza tra i detail del webhook e quelli della timeline con la versione "V30"
    #TEST VERIFICA REFINEMENT
    And vengono letti gli eventi fino all'elemento di timeline della notifica "REFINEMENT"
    Then vengono letti gli eventi dello stream del "Comune_1" fino all'elemento di timeline "REFINEMENT" con la versione "V30" e apiKey aggiornata con position 0
    #TODO: fixato lo step di sopra, il test fallirebbe quando confronta il timestamp di questi elementi
    And Si verifica che l'elemento di timeline "SCHEDULE_REFINEMENT" "abbia" il timestamp uguale a quello di "REFINEMENT" presente nel webhook con la versione "V30"
  ...
  Scenario: [B2B-STREAM_ES1.3_127] Consumo di uno stream notifica con gruppo, con eventType "STATUS"  utilizzando un apikey con stesso gruppo.
    ...
    And si predispone 1 nuovo stream denominato "stream-test" con eventType "STATUS" con versione "V30"
    And Viene creata una nuova apiKey per il comune "Comune_1" con il primo gruppo disponibile
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    And si crea il nuovo stream con versione "V30" per il "Comune_1" con un gruppo disponibile "FIRST"
  ...
  Scenario: [B2B-STREAM_ES1.1_158] Consumo di uno stream notifica con gruppi appartenenti ad un sottinsieme dei gruppi dell'apikey utilizzata.
    ...
    Then vengono letti gli eventi dello stream del "Comune_1" fino all'elemento di timeline "REQUEST_ACCEPTED" con la versione "V30" e apiKey aggiornata con position 0
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  @webhookV30 @precondition @cleanWebhook @webhook3
  Scenario: [B2B-STREAM_ES1.3_128] Consumo di uno stream notifica con gruppo, con eventType "TIMELINE"  utilizzando un apikey master.
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    And si crea il nuovo stream con versione "V30" per il "Comune_2" con un gruppo disponibile "FIRST"
    ...
    And viene impostata l'apikey appena generata
    Then vengono letti gli eventi dello stream del "Comune_2" fino all'elemento di timeline "REQUEST_ACCEPTED" con la versione "V30" e apiKey aggiornata con position 0
    #TEST DEANONIMIZZAZIONE
    And vengono letti gli eventi fino all'elemento di timeline della notifica "DIGITAL_SUCCESS_WORKFLOW"
    And vengono letti gli eventi dello stream del "Comune_2" fino all'elemento di timeline "SEND_COURTESY_MESSAGE" con la versione "V30" e apiKey aggiornata con position 0
    And verifica deanonimizzazione degli eventi di timeline versione "V30" senza delega digitale
    And vengono letti gli eventi dello stream del "Comune_2" fino all'elemento di timeline "SEND_DIGITAL_DOMICILE" con la versione "V30" e apiKey aggiornata con position 0
    And verifica deanonimizzazione degli eventi di timeline versione "V30" senza delega digitale
    And vengono letti gli eventi dello stream del "Comune_2" fino all'elemento di timeline "SEND_DIGITAL_FEEDBACK" con la versione "V30" e apiKey aggiornata con position 0
    And verifica deanonimizzazione degli eventi di timeline versione "V30" senza delega digitale
    And vengono letti gli eventi dello stream del "Comune_2" fino all'elemento di timeline "DIGITAL_SUCCESS_WORKFLOW" con la versione "V30" e apiKey aggiornata con position 0
    And verifica deanonimizzazione degli eventi di timeline versione "V30" senza delega digitale
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  Scenario: [B2B-STREAM_ES1.3_125_1] Consumo di uno stream notifica disabilitato senza gruppo, con eventType "STATUS"  utilizzando un apikey master (caso errato).
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    And si crea il nuovo stream per il "Comune_2" con versione "V30"
    And si disabilita lo stream "V30" creato per il comune "Comune_2"
    ...
    Then si verifica che non siano presenti eventi nello stream con versione "V30" del "Comune_2"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  Scenario: [B2B-STREAM_ES1.2_124] Verifica corretta scrittura degli eventi di una notifica creata con un apikey master, dove l’evento stesso deve essere salvato solo negli stream senza gruppi.
    ...
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
    And si predispone 1 nuovo stream denominato "stream-test1" con eventType "TIMELINE" con versione "V30"
    And Viene creata una nuova apiKey per il comune "Comune_2" senza gruppo
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    And si crea il nuovo stream per il "Comune_2" con versione "V30"
    And lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V30"
    And la notifica viene inviata tramite api b2b dal "Comune_2" e si attende che lo stato diventi "ACCEPTED"
    And vengono letti gli eventi dello stream del "Comune_2" fino all'elemento di timeline "REQUEST_ACCEPTED" con la versione "V30"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  Scenario: [B2B-STREAM_ES1.3_50_1] Consumo di uno stream notifica analogica senza gruppo, con eventType "TIMELINE"  utilizzando un apikey master e verifica corrispondenza tra i detail del webhook e quelli della timeline.
    ...
    Then vengono letti gli eventi dello stream del "Comune_Multi" fino all'elemento di timeline "PREPARE_ANALOG_DOMICILE" con la versione "V30" e apiKey aggiornata con position 0
    ...
    And verifica corrispondenza tra i detail del webhook e quelli della timeline con la versione "V30"
    And vengono letti gli eventi dello stream del "Comune_Multi" fino all'elemento di timeline "SEND_ANALOG_DOMICILE" con la versione "V30" e apiKey aggiornata con position 0
    And verifica corrispondenza tra i detail del webhook e quelli della timeline con la versione "V30"
    And vengono letti gli eventi dello stream del "Comune_Multi" fino all'elemento di timeline "ANALOG_SUCCESS_WORKFLOW" con la versione "V30" e apiKey aggiornata con position 0
    #TODO: lo step seguente fallisce a causa di una discrepanza tra i details b2b e quelli webhook (municipalityDetails B2b = "", municipalityDetails Webhook = null)
    And verifica corrispondenza tra i detail del webhook e quelli della timeline con la versione "V30"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  Scenario: [B2B-STREAM_ES3.1_146_2]  Lettura e verifica de-anonimizzazione con un apiKey con gruppo degli eventi di timeline di una notifica analogica inviata con un apikey con gruppo e salvati in uno stream dell'ente con gruppo (Stesso gruppo)
    ...
    And vengono letti gli eventi dello stream del "Comune_Multi" fino all'elemento di timeline "PREPARE_ANALOG_DOMICILE" con la versione "V30" e apiKey aggiornata con position 0
    And verifica deanonimizzazione degli eventi di timeline versione "V30" senza delega analogico
    And vengono letti gli eventi dello stream del "Comune_Multi" fino all'elemento di timeline "SEND_ANALOG_DOMICILE" con la versione "V30" e apiKey aggiornata con position 0
    And verifica deanonimizzazione degli eventi di timeline versione "V30" senza delega analogico
    And vengono letti gli eventi dello stream del "Comune_Multi" fino all'elemento di timeline "ANALOG_SUCCESS_WORKFLOW" con la versione "V30" e apiKey aggiornata con position 0
    And verifica deanonimizzazione degli eventi di timeline versione "V30" senza delega analogico
  ...
  Scenario: [B2B-STREAM_ES1.2_123] Creazione di stream con apiKey con gruppi differenti e verifica corretta scrittura degli eventi di notifiche create con le stesse apiKey.
    ...
    And vengono letti gli eventi dello stream del "Comune_Multi" fino all'elemento di timeline "REQUEST_ACCEPTED" con la versione "V30" e apiKey aggiornata con position 1
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
 ...
 #--------------LETTURA NUOVO EVENTO DI TIMELINE DI UNO STREAM------------DA ATTIVARE
  ...
  @webhookV30 @precondition @cleanWebhook @ignore
  Scenario: [B2B-STREAM_ES1.4_136] Lettura degli eventi di timeline  dello stream senza gruppo con visualizzazione del nuovo evento di timeline utilizzando un apikey abilitata.
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    And si crea il nuovo stream per il "Comune_1" con versione "V30"
    And la notifica viene inviata tramite api b2b dal "Comune_1" e si attende che lo stato diventi "ACCEPTED"
    ...
    Then vengono letti gli eventi dello stream del "Comune_1" fino all'elemento di timeline "NOTIFICATION_RADD_RETRIEVED" con la versione "V30" e apiKey aggiornata con position 0
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
 ...
 #SERVE INTEGRAZIONE CON RADD
  ...
  Scenario: [B2B-STREAM_ES1.4_137] Lettura degli eventi di timeline  dello stream con gruppo con visualizzazione  del nuovo evento di timeline utilizzando un apikey abilitata.
    ...
    When Viene creata una nuova apiKey per il comune "Comune_2" senza gruppo
    And viene impostata l'apikey appena generata
    Then vengono letti gli eventi dello stream del "Comune_2" fino all'elemento di timeline "NOTIFICATION_RADD_RETRIEVED" con la versione "V30" e apiKey aggiornata con position 0
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
 ...
 #SERVE INTEGRAZIONE CON RADD
  ...
  @webhookV30 @webhookHeader @precondition @cleanWebhook @webhook2
  Scenario Outline: [B2B-STREAM_RETRY_AFTER_V30] Creazione di stream con apiKey e controllo che il retry after dell'header venga modificato quando la consume restituisce elementi.
    Given viene generata una nuova notifica
      | subject               | notifica analogica con cucumber |
      | senderDenomination    | Comune di palermo               |
      | physicalCommunication | AR_REGISTERED_LETTER            |
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    And si crea il nuovo stream con versione "V30" per il "<paName>" con un gruppo disponibile "FIRST"
    When si effettua la consume dello stream versione "V30" salvando l'intera response
    Then l'header della response della consume con versione "V30" contiene il parametro "retry-after" con valore pari a "<retryAfterValue>"
    Given la notifica viene inviata tramite api b2b dal "<paName>" e si attende che lo stato diventi "ACCEPTED"
    And vengono letti gli eventi fino all'elemento di timeline della notifica "REQUEST_ACCEPTED"
    When si effettua la consume dello stream versione "V30" salvando l'intera response
    Then l'header della response della consume con versione "V30" contiene il parametro "retry-after" con valore pari a "0"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
    Examples:
