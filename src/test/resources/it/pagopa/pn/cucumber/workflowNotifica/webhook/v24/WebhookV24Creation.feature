Feature: verifica creazione stream
  ...
  Scenario: [B2B-STREAM_ES1.1_1] Creazione per una PA di 10 nuovi stream notifica con eventType TIMELINE e senza gruppo.
    Given vengono cancellati tutti gli stream presenti del "Comune_Multi" con versione "V24"
    And si predispongono 10 nuovo stream denominato "stream-test" con eventType "TIMELINE" con versione "V24"
    And Viene creata una nuova apiKey per il comune "Comune_Multi" senza gruppo
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    When si creano i nuovi stream per il "Comune_Multi" con versione "V24"
    Then lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V24"
    And si cancella lo stream creato per il "Comune_Multi" con versione "V24"
    And viene verificata la corretta cancellazione con versione "V24"
  ...
  Scenario: [B2B-STREAM_ES1.1_2] Creazione per una PA di 11 nuovi stream notifica con eventType TIMELINE e senza gruppo.
    Given vengono cancellati tutti gli stream presenti del "Comune_Multi" con versione "V24"
    And si predispone 10 nuovo stream denominato "stream-test" con eventType "TIMELINE" con versione "V24"
    And Viene creata una nuova apiKey per il comune "Comune_Multi" senza gruppo
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    And si creano i nuovi stream per il "Comune_Multi" con versione "V24"
    When si predispone 1 nuovo stream denominato "stream-test-11" con eventType "TIMELINE" con versione "V24"
    And si crea il nuovo stream per il "Comune_Multi" con versione "V24" (caso errato)
  ...
  Scenario: [B2B-STREAM_ES1.1_3] Creazione per una PA di 10 nuovi stream notifica con eventType TIMELINE con gruppo.
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    When si crea il nuovo stream con versione "V24" per il "Comune_Multi" con un gruppo disponibile "FIRST"
    Then lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V24"
    And si cancella lo stream creato per il "Comune_Multi" con versione "V24"
    And viene verificata la corretta cancellazione con versione "V24"
    And viene modificato lo stato dell'apiKey in "BLOCK"
  ...
  Scenario: [B2B-STREAM_ES1.1_5] Creazione per una PA di 10 nuovi stream notifica con eventType STATUS e senza gruppo.
    Given vengono cancellati tutti gli stream presenti del "Comune_Multi" con versione "V24"
    And si predispone 10 nuovo stream denominato "stream-test" con eventType "STATUS" con versione "V24"
    And Viene creata una nuova apiKey per il comune "Comune_Multi" senza gruppo
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    When si crea il nuovo stream per il "Comune_Multi" con versione "V24"
    Then lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V24"
    And si cancella lo stream creato per il "Comune_Multi" con versione "V24"
    And viene verificata la corretta cancellazione con versione "V24"
    And viene modificato lo stato dell'apiKey in "BLOCK"
  ...
  Scenario: [B2B-STREAM_ES1.1_6] Creazione per una PA di 11 nuovi stream notifica con eventType STATUS e senza gruppo.
    ...
    And viene aggiornata la apiKey utilizzata per gli stream
    And si creano i nuovi stream per il "Comune_Multi" con versione "V24"
    When si predispone 1 nuovo stream denominato "stream-test-11" con eventType "STATUS" con versione "V24"
    And si crea il nuovo stream per il "Comune_Multi" con versione "V24" (caso errato)
    Then l'operazione ha prodotto un errore con status code "409"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  Scenario: [B2B-STREAM_ES1.1_9] Creazione di uno stream notifica con gruppo, con eventType "STATUS"  utilizzando un apikey master.
    ...
    And viene aggiornata la apiKey utilizzata per gli stream
    When si crea il nuovo stream con versione "V24" per il "Comune_Multi" con un gruppo disponibile "FIRST"
    Then si cancella lo stream creato per il "Comune_Multi" con versione "V24"
    And viene verificata la corretta cancellazione con versione "V24"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  Scenario: [B2B-STREAM_ES1.1_11] Creazione di uno stream notifica con gruppo non appartenente alla PA dell'apiKey, con eventType "STATUS"  utilizzando un apikey con gruppo.
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    When si crea il nuovo stream con versione "V24" per il "Comune_Multi" con un gruppo disponibile "ALTRA_PA" (caso errato)
  ...
  Scenario: [B2B-STREAM_ES1.1_12] Creazione di uno stream notifica con gruppo, con eventType "STATUS"  utilizzando un apikey con gruppo diverso.
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    When si crea il nuovo stream con versione "V24" per il "Comune_Multi" con un gruppo disponibile "LAST" (caso errato)
    Then l'operazione ha prodotto un errore con status code "403"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  Scenario: [B2B-STREAM_ES1.1_21] Creazione di uno stream notifica senza gruppo, con eventType "STATUS"  utilizzando un apikey con gruppo.
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    When si crea il nuovo stream con versione "V24" per il "Comune_Multi" con un gruppo disponibile "NO_GROUPS" (caso errato)
    Then l'operazione ha prodotto un errore con status code "403"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  Scenario: [B2B-STREAM_ES1.1_148] Creazione di uno stream notifica con gruppi appartenenti ad un sottinsieme dei gruppi dell'apikey utilizzata.
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    When si crea il nuovo stream con versione "V24" per il "Comune_Multi" con un gruppo disponibile "LAST"
    Then lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V24"
    And si cancella lo stream creato per il "Comune_Multi" con versione "V24"
    And viene verificata la corretta cancellazione con versione "V24"
    And viene modificato lo stato dell'apiKey in "BLOCK"
  ...
  Scenario: [B2B-STREAM_ES1.1_149] Creazione di uno stream notifica con gruppi non appartenenti ad un sottinsieme dei gruppi dell'apikey utilizzata.
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    When si crea il nuovo stream con versione "V24" per il "Comune_Multi" con un gruppo disponibile "ALL" (caso errato)
    Then l'operazione ha prodotto un errore con status code "403"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
