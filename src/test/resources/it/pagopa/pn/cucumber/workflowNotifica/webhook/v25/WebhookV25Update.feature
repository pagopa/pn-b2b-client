Feature: aggiornamento stream
  ...
  Scenario: [B2B-STREAM_ES1.1_87] Aggiornamento di uno stream notifica con gruppo, con eventType "STATUS"  con gruppo non appartenente alla PA.
    ...
    When si aggiorna lo stream creato con versione "V25" con un gruppo che non appartiene al comune "Comune_Multi"
  ...
  Scenario: [B2B-STREAM_ES1.1_88] Aggiornamento di uno stream notifica con gruppo, con eventType "STATUS"  utilizzando un apikey con gruppo diverso.
    ...
    And viene impostata l'apikey appena generata
    When si aggiorna lo stream creato con versione "V25" e apiKey aggiornata
    Then l'operazione ha prodotto un errore con status code "403"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  Scenario: [B2B-STREAM_ES1.1_42] Aggiornamento di uno stream notifica da due gruppi ad un gruppo, con eventType "STATUS".
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    When si crea il nuovo stream con versione "V25" per il "Comune_Multi" con un gruppo disponibile "UGUALI"
    And lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V25"
    When si "rimuove" un gruppo allo stream creato con versione "V25" per il comune "Comune_Multi" e apiKey aggiornata
    Then l'operazione ha prodotto un errore con status code "403"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  Scenario: [B2B-STREAM_ES1.1_43] Aggiornamento di uno stream notifica da un gruppo a più gruppi, con eventType "STATUS".
    ...
    When si crea il nuovo stream con versione "V25" per il "Comune_Multi" con un gruppo disponibile "UGUALI"
    And lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V25"
    When si "aggiunge" un gruppo allo stream creato con versione "V25" per il comune "Comune_Multi" e apiKey aggiornata
    Then l'operazione non ha prodotto errori
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  Scenario: [B2B-STREAM_ES1.1_91] Aggiornamento di uno stream notifica con gruppo, con eventType "TIMELINE"  utilizzando un apikey master.
    ...
    And Viene creata una nuova apiKey per il comune "Comune_Multi" senza gruppo
    And viene impostata l'apikey appena generata
    When si "stesso" un gruppo allo stream creato con versione "V25" per il comune "Comune_Multi" e apiKey aggiornata
    Then l'operazione non ha prodotto errori
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
  ...
  Scenario: [B2B-STREAM_ES1.1_154] Aggiornamento di uno stream notifica con gruppi appartenenti ad un sottinsieme dei gruppi dell'apikey utilizzata.
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    And si crea il nuovo stream con versione "V25" per il "Comune_Multi" con un gruppo disponibile "LAST"
    And lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V25"
    When si aggiorna lo stream creato con versione "V25" e apiKey aggiornata
    Then l'operazione non ha prodotto errori
    And viene modificato lo stato dell'apiKey in "BLOCK"
  ...
  Scenario: [B2B-STREAM_ES1.1_155] Aggiornamento di uno stream notifica con gruppi appartenenti ad un sottinsieme dei gruppi dell'apikey utilizzata.
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    And si crea il nuovo stream con versione "V25" per il "Comune_Multi" con un gruppo disponibile "UGUALI"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
    And Viene creata una nuova apiKey per il comune "Comune_Multi" con il primo gruppo disponibile
    And viene impostata l'apikey appena generata
  ...
  Scenario: [B2B-STREAM_ES1.1_121] Aggiornamento per una PA di uno stream che non esiste per la stessa PA
    ...
    And si crea il nuovo stream per il "Comune_Multi" con versione "V25"
    And lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V25"
    When si cancella lo stream creato per il "Comune_Multi" con versione "V25"
    Then si aggiorna lo stream che non esiste e apiKey aggiornata con versione "V25"
  ...
  Scenario: [B2B-STREAM_ES1.5.138] Aggiornamento di uno stream notifica V25 senza gruppo, con  la versione V10.
    ...
    And si crea il nuovo stream per il "Comune_Multi" con versione "V25"
    And lo stream è stato creato e viene correttamente recuperato dal sistema tramite stream id con versione "V25"
    When si aggiorna lo stream creato con versione "V25" invocando la versione "V10" - Cross Versioning
    Then l'operazione ha prodotto un errore con status code "400"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata

  @webhookV25 @precondition @cleanWebhook @webhook2
  Scenario: [B2B-STREAM_ES1.5.140] Aggiornamento di uno stream notifica V10 senza gruppo, con  la versione V25.
    ...
    And Viene creata una nuova apiKey per il comune "Comune_Multi" senza gruppo
    And viene impostata l'apikey appena generata
    When si aggiorna lo stream creato con versione "V10" invocando la versione "V25" - Cross Versioning
    Then l'operazione ha prodotto un errore con status code "403"
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
