Feature: avanzamento notifiche b2b persona giuridica multi pagamento
 ...
 #24 PA - inserimento notifica mono destinatario con un solo avviso pagoPA [TA]
  ...
  @pagamentiMultipli
  Scenario: [B2B-PA-PAY_MULTI_PG_9422] PA - inserimento notifica mono destinatario con un solo avviso pagoPA e costi di notifica  inclusi modalità DELIVERY_MODE (scenario positivo) - senza allegato Avviso PagoPA
    Given viene generata una nuova notifica
      | subject            | invio notifica con cucumber |
      | senderDenomination | Comune di Palermo           |
      | feePolicy          | DELIVERY_MODE               |
