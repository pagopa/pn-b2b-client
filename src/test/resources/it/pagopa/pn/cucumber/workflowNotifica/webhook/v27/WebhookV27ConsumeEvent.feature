Feature: avanzamento notifiche webhook b2b V27
  ...
  Scenario: [B2B-STREAM_ES1.3_128] Consumo di uno stream notifica con gruppo, con eventType "TIMELINE"  utilizzando un apikey master.
    ...
    And viene impostata l'apikey appena generata
    Then vengono letti gli eventi dello stream del "Comune_2" fino all'elemento di timeline "REQUEST_ACCEPTED" con la versione "V27" e apiKey aggiornata con position 0
 ...
 #--------------LETTURA NUOVO EVENTO DI TIMELINE DI UNO STREAM------------DA ATTIVARE
  ...
  Scenario: [B2B-STREAM_ES1.4_136] Lettura degli eventi di timeline  dello stream senza gruppo con visualizzazione del nuovo evento di timeline utilizzando un apikey abilitata.
    ...
    Then vengono letti gli eventi dello stream del "Comune_1" fino all'elemento di timeline "NOTIFICATION_RADD_RETRIEVED" con la versione "V27" e apiKey aggiornata con position 0
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
 ...
 #SERVE INTEGRAZIONE CON RADD
  ...
  Scenario: [B2B-STREAM_ES1.4_137] Lettura degli eventi di timeline  dello stream con gruppo con visualizzazione  del nuovo evento di timeline utilizzando un apikey abilitata.
    ...
    When Viene creata una nuova apiKey per il comune "Comune_2" senza gruppo
    And viene impostata l'apikey appena generata
    Then vengono letti gli eventi dello stream del "Comune_2" fino all'elemento di timeline "NOTIFICATION_RADD_RETRIEVED" con la versione "V27" e apiKey aggiornata con position 0
    And viene modificato lo stato dell'apiKey in "BLOCK"
    And l'apiKey viene cancellata
 ...
 #SERVE INTEGRAZIONE CON RADD
  ...
  Scenario Outline: [B2B-STREAM_RETRY_AFTER_V27] Creazione di stream con apiKey e controllo che il retry after dell'header venga modificato quando la consume restituisce elementi.
    ...
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
    And si crea il nuovo stream con versione "V27" per il "<paName>" con un gruppo disponibile "FIRST"
    When si effettua la consume dello stream versione "V27" salvando l'intera response
    Then l'header della response della consume con versione "V27" contiene il parametro "retry-after" con valore pari a "<retryAfterValue>"
