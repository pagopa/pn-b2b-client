Feature: tentativo consumo stream
  ...
  @webhookV29 @precondition @cleanWebhook @webhook2
  Scenario: [B2B-STREAM_ES1.5_139] Creazione di uno stream senza gruppo con la V29 e lettura Eventi di timeline o di cambio di stato con la versione V10  utilizzando un apikey abilitata
    Given si predispone 1 nuovo stream denominato "stream-test" con eventType "TIMELINE" con versione "V29"
    And Viene creata una nuova apiKey per il comune "Comune_Multi" senza gruppo
    And viene impostata l'apikey appena generata
    And viene aggiornata la apiKey utilizzata per gli stream
  ...
  @webhookV29 @precondition @cleanWebhook @webhook2
  Scenario: [B2B-STREAM_ES1.5_141] Creazione di uno stream senza gruppo con la V10 e  lettura Eventi di timeline o di cambio di stato con la versione V29 utilizzando un apikey abilitata.
