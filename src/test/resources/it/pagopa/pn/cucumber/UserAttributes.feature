Feature: Attributi utente
  ...
  Scenario: [USER-ATTR_10] inserimento email di cortesia e controllo presenza per ente figlio
    Given si predispone addressbook per l'utente "Mario Cucumber"
    When viene richiesto l'inserimento del email di cortesia "provaemail@test.it" per comune "comune_Son"
    And  l'inserimento ha prodotto un errore con status code "400"


  @AOO_UO
  Scenario: [USER-ATTR_10_PG] inserimento email di cortesia a PG e controllo presenza per ente figlio
