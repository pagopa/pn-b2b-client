...
final class EserviceInterfaceUploadOps implements DocumentUploadOps {

    private final SharedStepsContext sharedStepsContext;
    private final IM2MEserviceDescriptorClient descriptorClient;
    private final PollingService pollingService;

    EserviceInterfaceUploadOps(
    ...
    public UploadOperationResult upload(UploadRequest request, Resource uploadResource, String prettyName) {
        ...
        HttpStatus uploadStatus = sharedStepsContext.getHttpCallExecutor().performCall(
            () -> descriptorClient.uploadInterface(eServiceId, descriptorId, uploadResource, prettyName)
        );
    ...
    public void pollDocumentAvailability(UUID documentId) {
        ...
        pollingService.makePolling(
            () -> sharedStepsContext.getHttpCallExecutor().performCall(
                () -> descriptorClient.downloadEServiceDescriptorInterface(eServiceId, descriptorId)
            ),
            HttpStatus.OK::equals,
            status -> "downloadEServiceDescriptorInterface non disponibile, status=" + status
        );
    }
}
