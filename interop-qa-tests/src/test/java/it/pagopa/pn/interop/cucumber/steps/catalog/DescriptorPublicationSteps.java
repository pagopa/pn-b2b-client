...
public class DescriptorPublicationSteps {
    ...
    public void publishDescriptor(IHttpExecutor httpExecutor, IEServiceClient client, EServicesCommonContext context) {
        sharedStepsContext.getPollingService().makePolling(
                () -> httpExecutor.performCall(() ->
                        client.publishDescriptor(context.getEserviceId(), context.getDescriptorId())
                ),
                res -> httpExecutor.getResponseStatus().is2xxSuccessful(),
