...
Feature: (M2M v3) Gestione deleghe per archiviazione manuale e-service
  ...
  @sad-path
  Scenario: [M2M_V3_DELEGATION_MANUAL_ARCHIVING_4.4] Un utente con ruolo m2m NON può annullare via M2M v3 la richiesta di archiviazione di un descrittore diverso dal più recente precedentemente inviata e ancora in pending
    Given l'ente delegante "PA1"
    And l'ente delegato "PA2"
    And "PA1" ha già creato un e-service con un descrittore in stato "PUBLISHED"
    And "PA3" ha una richiesta di fruizione in stato "ACTIVE" per quell'e-service
  ...
  @happy-path
  Scenario: [M2M_V3_DELEGATION_MANUAL_ARCHIVING_5.1] Un ente delegato può inviare via M2M v3 una richiesta di archiviazione dell'e-service al delegante a seguito dell'annullamento del processo di archiviazione
...
#     TODO: da sostituire con il Given
  ...
  Scenario Outline: [M2M_V3_DELEGATION_MANUAL_ARCHIVING_CONTRACT_2.3] Specificando parametri errati o mancanti, un ente delegante NON può accettare via M2M v3 la richiesta di archiviazione del descrittore meno recente inviata dall'ente delegato
    ...
    And l'utente ha già inviato la richiesta di archiviazione per il vecchio descrittore "%actual" dell'e-service "%actual" specificando 60 giorni di preavviso
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
    When l'utente delegante accetta via M2M v3 la richiesta di archiviazione della vecchia versione identificata da "<descriptorId>" per l'e-service "<eServiceId>"
    Then si ottiene response status code <statusCode>
  ...
  @sad-path
  Scenario: [M2M_V3_DELEGATION_MANUAL_ARCHIVING_CONTRACT_2.4] Un ente delegante NON può accettare via M2M v3 la richiesta di archiviazione del descrittore meno recente inviata dall'ente delegato se il token di accesso utilizzato non è valido
    Given l'ente delegante "PA1"
    And l'ente delegato "PA2"
    And "PA1" ha già creato un e-service con un descrittore in stato "PUBLISHED"
    And "PA3" ha una richiesta di fruizione in stato "ACTIVE" per quell'e-service
  ...
  @sad-path
  Scenario Outline: [M2M_V3_DELEGATION_MANUAL_ARCHIVING_CONTRACT_3.3] Specificando parametri errati o mancanti, un ente delegante NON può rifiutare via M2M v3 la richiesta di archiviazione del descrittore meno recente inviata dall'ente delegato
    Given l'ente delegante "PA1"
    And l'ente delegato "PA2"
    And "PA1" ha già creato un e-service con un descrittore in stato "PUBLISHED"
    And "PA3" ha una richiesta di fruizione in stato "ACTIVE" per quell'e-service
    ...
    And l'utente ha già inviato la richiesta di archiviazione per il vecchio descrittore "%actual" dell'e-service "%actual" specificando 60 giorni di preavviso
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
    When l'utente delegante rifiuta via M2M v3 la richiesta di archiviazione della vecchia versione identificata da "<descriptorId>" per l'e-service "<eServiceId>" con motivazione "<rejectionReason>"
    Then si ottiene response status code <statusCode>
  ...
  @sad-path
  Scenario: [M2M_V3_DELEGATION_MANUAL_ARCHIVING_CONTRACT_3.4] Un ente delegante NON può rifiutare via M2M v3 la richiesta di archiviazione del descrittore meno recente inviata dall'ente delegato se il token di accesso utilizzato non è valido
    Given l'ente delegante "PA1"
    And l'ente delegato "PA2"
    And "PA1" ha già creato un e-service con un descrittore in stato "PUBLISHED"
    And "PA3" ha una richiesta di fruizione in stato "ACTIVE" per quell'e-service
    ...
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
    And viene impostato per l'utente un token m2m non valido
    When l'utente delegante rifiuta via M2M v3 la richiesta di archiviazione della vecchia versione identificata da "%actual" per l'e-service "%actual" con motivazione "QA test rejection delegation archiving"
    Then si ottiene response status code 401

  @sad-path
  Scenario Outline: [M2M_V3_DELEGATION_MANUAL_ARCHIVING_CONTRACT_4.1] Specificando parametri errati o mancanti, l'ente delegato NON può annullare via M2M v3 la richiesta di archiviazione dell'e-service precedentemente inviata e ancora in pending
    Given l'ente delegante "PA1"
    And l'ente delegato "PA2"
    And "PA1" ha già creato un e-service con un descrittore in stato "PUBLISHED"
    And l'ente "PA2" concede la disponibilità a ricevere deleghe in erogazione
    ...
    And l'utente ha già inviato la richiesta di archiviazione per l'e-service "%actual" specificando la motivazione "QA test delegation manual archiving" e 60 giorni di preavviso
    And l'utente è un "admin" di "PA2" con ruolo M2M m2m-admin
    When l'utente delegato annulla via M2M v3 la richiesta di archiviazione dell'e-service "<eServiceId>"
    Then si ottiene response status code <statusCode>
  ...
  @sad-path
  Scenario: [M2M_V3_DELEGATION_MANUAL_ARCHIVING_CONTRACT_4.2] L'ente delegato NON può annullare via M2M v3 la richiesta di archiviazione dell'e-service precedentemente inviata e ancora in pending se il token di accesso utilizzato non è valido
    Given l'ente delegante "PA1"
    And l'ente delegato "PA2"
    And "PA1" ha già creato un e-service con un descrittore in stato "PUBLISHED"
    And l'ente "PA2" concede la disponibilità a ricevere deleghe in erogazione
