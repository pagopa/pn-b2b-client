...
Feature: Gestione di assegnazione degli attributi certificati discreti ai tenant attraverso APIs M2M V3
  ...
  Scenario Outline: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_TENANTS_1] L'elenco degli attributi certificati discreti associati ad un ente va a buon fine.
  ...
  Scenario: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_TENANTS_2] Il tentativo di recuperare gli attributi certificati discreti associati a un ente fallisce se il relativo UUID non è valido.
    Given l'utente è un "admin" di "GSP" con ruolo M2M m2m-admin
    When l'utente richiede l'elenco degli attributi certificati discreti di "PA1" utilizzando un UUID invalido
    Then si ottiene lo status code 404

  Scenario: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_TENANTS_3] La richiesta dell'elenco degli attributi certificati discreti associati all'utente non va a buon fine se i parametri utilizzati non sono validi.
    Given l'utente è un "admin" di "GSP" con ruolo M2M m2m-admin
  ...
  Scenario: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_TENANTS_4] Il tentativo di recuperare gli attributi certificati discreti associati a un ente fallisce se il relativo UUID non esiste.
    Given l'utente è un "admin" di "GSP" con ruolo M2M m2m-admin
    When l'utente richiede l'elenco degli attributi certificati discreti di "PA1" utilizzando un UUID inesistente
    Then si ottiene lo status code 404

  Scenario: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_TENANTS_5] Il tentativo di recuperare gli attributi certificati discreti associati all'ente non va a buon fine se il token di autenticazione non è valido.
    Given l'utente è un "admin" di "GSP" con ruolo M2M m2m-admin
    And viene effettuata la creazione dell'attributo certificato discreto con successo
      | name | description | code |
      |      |             |      |
  ...
  Scenario Outline: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_TENANTS_CREATE_1] Gli attributi certificati discreti non possono essere associati dagli utenti non autorizzati.
    Given l'utente è un "admin" di "GSP" con ruolo M2M m2m-admin
    And viene effettuata la creazione dell'attributo certificato discreto con successo
      | name | description | code |
      |      |             |      |
  ...
  Scenario: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_TENANTS_DELETE_1] L'operazione di revoca di un attributo certificato discreto va a buon fine se l'utente è autorizzato.
    ...
    And l'utente richiede l'elenco degli attributi certificati discreti di "PA1" e il sistema restituisce correttamente gli attributi associati e quelli revocati
    Then si ottiene lo status code 200

  Scenario: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_TENANTS_DELETE_2] L'operazione di revoca di un attributo certificato discreto non va a buon fine se l'utente non è autorizzato.
    Given l'utente è un "admin" di "GSP" con ruolo M2M m2m-admin
    And viene effettuata la creazione dell'attributo certificato discreto con successo
      | name | description | code |
      |      |             |      |
  ...
  Scenario: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_FUNC_REVOKE_1] L'accordo di fruizione che richiede l'attributo viene automaticamente sospeso dal sistema se lo stesso viene revocato al fruitore.
    Given l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
    And "PA1" ha già creato un e-service in stato "DRAFT" che richiede quegli attributi con approvazione "AUTOMATIC" con dailyCallsPerConsumer uguale a 100 e dailyCallsTotal uguale a 1000
    ...
    And l'utente assegna a "PA2" l'attributo certificato discreto creato con un valore discreto di 5 con successo
    And l'utente è un "admin" di "PA2"
    And "PA2" ha una richiesta di fruizione in stato "ACTIVE" per quell'e-service
    And l'utente crea una nuova finalità per quell'e-service con tutti i campi richiesti correttamente formattati e con dailyCalls uguale a 15
    ...
    Then la richiesta di fruizione è in stato "SUSPENDED"
  ...
  Scenario: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_FUNC_UDPATE_1] L'operazione di aggiornamento di un attributo certificato discreto non va a buon fine se lo stesso è già stato revocato all'ente.
    Given l'utente è un "admin" di "GSP" con ruolo M2M m2m-admin
    And viene effettuata la creazione dell'attributo certificato discreto con successo
      | name | description | code |
      |      |             |      |
