...
Feature: Revoca di un attributo certificato posseduto da uno specifico aderente
  ...
  @tenant_revoke_certified_attribute1 @no-parallel @certifiedAttribute
...
#      | support      |        403 |
  ...
  Scenario: Per un attributo certificato precedentemente creato da un primo aderente, il quale ha la qualifica di ente certificatore (certifier), che lo assegna ad un secondo ente, alla richiesta di revoca da parte di un utente con sufficienti permessi (admin) appartenente ad un terzo ente certificatore, ottiene un errore
