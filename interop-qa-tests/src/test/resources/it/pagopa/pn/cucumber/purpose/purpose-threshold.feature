...
Feature: Verifica soglie differenziate
  ...
  @dailyCallsThreshold
  Scenario: [PURPOSE_THRESHOLD_2] Per la creazione di una finalità il sistema attribuisce la soglia di default se il fruitore non possiede nessun attributo certificato associato ad una soglia
    Given l'utente è un "admin" di "PA2"
    ...
    And "PA2" ha già creato un e-service in stato "PUBLISHED" che richiede quegli attributi con approvazione "AUTOMATIC" e:
      | dailyCallsPerConsumer | 10   |
      | dailyCallsTotal       | 1000 |
    And l'utente tenta di aggiungere una soglia differenziata di 100 per l'attributo "certificato" 0-esimo creato
    And si ottiene status code 200
    And "PA1" ha una richiesta di fruizione in stato "ACTIVE" per quell'e-service
  ...
  @dailyCallsThreshold
  Scenario: [PURPOSE_THRESHOLD_3] Per la creazione di una finalità il sistema attribuisce la soglia maggiore degli attributi certificati
    Given l'utente è un "admin" di "PA2"
    And PA2 ha già creato 2 attributo CERTIFIED
    And l'utente assegna a "PA1" gli attributi certificati precedentemente creati
    And si ottiene status code 200
    ...
    And l'utente tenta di aggiungere una soglia differenziata di 100 per l'attributo "certificato" 0-esimo creato
    And si ottiene status code 200
    And l'utente tenta di aggiungere una soglia differenziata di 1000 per l'attributo "certificato" 1-esimo creato
    And si ottiene status code 200
    And "PA1" ha una richiesta di fruizione in stato "ACTIVE" per quell'e-service
  ...
  Scenario: [PURPOSE_THRESHOLD_4] Per la creazione di una finalità il sistema calcola correttamente il campo remainingDailyCallsPerConsumer, scalandolo per le purpose provenienti dallo stesso fruitore per lo stesso eservice e non scalandolo per le purpose provenienti da altri fruitori per lo stesso eservice, e remainingDailyCallsTotal scalandolo per ogni purpose attiva
    ...
    And "PA2" ha già creato un e-service in stato "PUBLISHED" che richiede quegli attributi con approvazione "AUTOMATIC" e:
      ...
      | dailyCallsTotal       | 15 |
    And l'utente tenta di aggiungere una soglia differenziata di 10 per l'attributo "certificato" 0-esimo creato
    And si ottiene status code 200
    And "PA1" ha una richiesta di fruizione in stato "ACTIVE" per quell'e-service
  ...
  @dailyCallsThreshold
  Scenario: [PURPOSE_THRESHOLD_5] Per la creazione di una finalità il sistema demanda allo stato WAITING_FOR_APROVAL se il limite di chiamate giornaliere per fruitore supera il limite fornito dall'erogatore
    Given l'utente è un "admin" di "PA2"
    And PA2 ha già creato 1 attributo CERTIFIED
    And l'utente assegna a "PA1" gli attributi certificati precedentemente creati
    And si ottiene status code 200
  ...
  @dailyCallsThreshold
  Scenario: [PURPOSE_THRESHOLD_6] Una finalità in stato ACTIVE è indifferente ai nuovi cambiamenti delle soglie associate agli attributi certificati posseduti per la fruizione di un certo eservice
    Given l'utente è un "admin" di "PA2"
    And PA2 ha già creato 1 attributo CERTIFIED
    And l'utente assegna a "PA1" gli attributi certificati precedentemente creati
    And si ottiene status code 200
    ...
    When l'utente tenta di aggiungere una soglia differenziata di 1 per l'attributo "certificato" 0-esimo creato
    And l'utente è un "admin" di "PA1"
  ...
  @dailyCallsThreshold
  Scenario: [PURPOSE_THRESHOLD_7] Una finalità in stato ACTIVE è alla revoca di un attributo certificato non essenziale per la fruizione ma associato ad una certa soglia
    Given l'utente è un "admin" di "PA2"
    And PA2 ha già creato 2 attributo CERTIFIED
    And l'utente assegna a "PA1" l'attributo certificato precedentemente creato
    And si ottiene status code 200
