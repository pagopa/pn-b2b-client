...
#      | GSP  | api,security |
  ...
  @happy-path
  @descriptor_import3
  Scenario: La richiesta di import di un descrittore di un e-service in erogazione inversa, dato un pacchetto correttamente strutturato, va a buon fine e il descrittore viene correttamente creato in stato DRAFT con l’analisi del rischio fornita dal pacchetto
  ...
  Scenario: La richiesta di import di un descrittore di un e-service, dato un pacchetto con il file di configurazione correttamente formattato ma con il nome del file errato, non va a buon fine.
    Given l'utente è un "admin" di "PA1"
    Given l'utente ha già un pacchetto correttamente strutturato con un eservice in mode "DELIVER"
    Given l'utente ha già richiesto una presignedURL per il caricamento del pacchetto
    Given è già stato caricato il pacchetto nella presignedURL
  ...
  @sad-path
  @descriptor_import5
  Scenario: La richiesta di import di un descrittore di un e-service, dato un pacchetto con il file di configurazione non correttamente formattato (campi richiesti mancanti o json non valido), non va a buon fine
    Given l'utente è un "admin" di "PA1"
  ...
  Scenario: La richiesta di import di un descrittore di un e-service, dato un pacchetto con il file di configurazione correttamente formattato ma contenente documenti (o file di interfaccia) che non esistono nel percorso previsto, non va a buon fine
    Given l'utente è un "admin" di "PA1"
  ...
  @sad-path
  @descriptor_import7
  Scenario: La richiesta di import di un descrittore di un e-service, dato un pacchetto con il file di configurazione correttamente formattato, ma con file in cartella non previsti all’interno del file di configurazione, non va a buon fine
    Given l'utente è un "admin" di "PA1"
