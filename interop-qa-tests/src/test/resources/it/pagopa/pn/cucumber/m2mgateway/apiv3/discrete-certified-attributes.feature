Feature: Gestione degli attributi certificati discreti attraverso APIs M2M V3
  ...
  Scenario: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_1] La creazione di un attributo certificato discreto va a buon fine
  se l'utente è un admin di un ente certificatore.
  ...
  Scenario Outline: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_2] La creazione di un attributo certificato discreto non va a buon fine
  se uno dei suoi attributi non è valido.
  ...
  Scenario: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_3] La creazione di un attributo certificato discreto non va a buon fine se
  la richiesta http non è valida.
    Given l'utente è un "admin" di "GSP" con ruolo M2M m2m-admin
    When si tenta la creazione dell'attributo certificato discreto senza passare parametri nella richiesta
    Then si ottiene lo status code 400

  Scenario: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_4] Accesso negato al dettaglio di un attributo certificato discreto con
  token non valido.
  ...
  Scenario: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_5] La creazione di un attributo certificato discreto non va a buon fine se
  ce n'è già uno con lo stesso nome.
  ...
  Scenario Outline: [M2M_CERTIFIED_DISCRETE_ATTRIBUTES_6] La creazione di un attributo certificato discreto non è consentita
  alle utenze che non sono admin o che non appartengano agli enti certificatori.
