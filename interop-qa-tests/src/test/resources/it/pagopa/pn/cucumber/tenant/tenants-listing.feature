...
Feature: Listing degli aderenti
  ...
  Scenario Outline: A fronte di 3 aderenti in db, restituisce solo i primi 2 risultati (scopo del test è verificare il corretto funzionamento del parametro limit)
    ...
    @tenants_listing2
  Scenario: Restituisce gli aderenti che contengono la keyword "comune di Milano" all'interno del nome, con ricerca case insensitive (scopo del test è verificare che funzioni il filtro name)
