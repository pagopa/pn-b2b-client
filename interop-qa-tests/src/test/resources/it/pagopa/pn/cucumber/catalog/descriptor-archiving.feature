...
#      firstDescriptorId=descrittore in Archiving_Suspended . secondDescriptorId=descrittore in Published
  ...
  Scenario: [MANUAL_ARCHIVING_DESCRIPTOR_CLONING_1.4] L'ente erogatore può clonare un descrittore archiviato al termine del processo di archiviazione manuale
    Given l'utente è un "admin" di "PA1"
    When l'utente tenta di clonare il descrittore con id "ee9dbc42-289f-400d-86f8-51e8fb704933" dell'e-service con id "45d822b9-066c-4a77-b4a1-331135496907"
    Then si ottiene response status code 200
    And l'e-service è stato clonato con successo
    And l'e-service è in stato "DRAFT"
    And il descrittore più recente non è stato messo in archiviazione tramite l'archiviazione manuale del singolo descrittore
  ...
  @happy-path
  Scenario Outline: [MANUAL_ARCHIVING_DESCRIPTOR_ASYNC_1.2] Un ente erogatore di un e-service asincrono può avviare il processo di archiviazione manuale del primo e meno recente descrittore in stato SUSPENDED
    Given l'utente è un "admin" di "PA2"
    And "PA2" ha già creato un e-service asincrono con un descrittore in stato "PUBLISHED" con:
      | asyncExchangeProperties.responseTime          | 100  |
      | asyncExchangeProperties.resourceAvailableTime | 100  |
  ...
  @happy-path
  Scenario Outline: [MANUAL_ARCHIVING_DESCRIPTOR_ASYNC_TEMPLATE_INSTANCE_ELIMINATION_1.1] L'ente erogatore di un e-service asincrono creato da template può annullare l'archiviazione manuale in corso di un descrittore precedentemente in stato DEPRECATED
    Given l'utente è un "admin" di "PA1"
    And l'utente effettua la creazione di un e-service template asincrono in modalità erogazione con tecnologia "REST" in stato di PUBLISHED
    And l'utente effettua la creazione di un nuovo e-service in stato PUBLISHED a partire dal template con successo indicando solo le specifiche strettamente necessarie
    And "PA2" ha una richiesta di fruizione in stato "ACTIVE" per quell'e-service
  ...
  @happy-path
  Scenario Outline: [MANUAL_ARCHIVING_DESCRIPTOR_ASYNC_TEMPLATE_INSTANCE_ELIMINATION_1.2] L'ente erogatore di un e-service creato da template può annullare l'archiviazione manuale in corso di un descrittore in stato ARCHIVING_SUSPENDED
    Given l'utente è un "admin" di "PA1"
    And l'utente effettua la creazione di un e-service template asincrono in modalità erogazione con tecnologia "REST" in stato di PUBLISHED
    And l'utente effettua la creazione di un nuovo e-service in stato PUBLISHED a partire dal template con successo indicando solo le specifiche strettamente necessarie
    And "PA2" ha una richiesta di fruizione in stato "ACTIVE" per quell'e-service
