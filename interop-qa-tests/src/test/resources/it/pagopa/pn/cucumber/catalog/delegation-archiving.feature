...
Feature: Gestione deleghe per archiviazione manuale e-service
  ...
  @happy-path
  Scenario Outline: [DELEGATION_MANUAL_ARCHIVING_4.4] Un utente appartente all'ente delegato ma con ruolo non ammesso NONvpuò annullare la richiesta di archiviazione del descrittore meno recente precedentemente inviata e ancora in pending
    Given l'ente delegante "PA1"
    And l'ente delegato "PA2"
    And "PA1" ha già creato un e-service con un descrittore in stato "PUBLISHED"
    And "PA3" ha una richiesta di fruizione in stato "ACTIVE" per quell'e-service
