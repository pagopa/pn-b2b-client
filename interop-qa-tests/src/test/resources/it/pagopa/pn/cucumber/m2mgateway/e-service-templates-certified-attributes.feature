@m2mEservices
@m2m-eservice-template-certified-attributes
Feature: Gestione degli attributi certificati degli e-service templates templates attraverso APIs M2M V2
  ...
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_ADD_01] Un utente con ruolo M2M-ADMIN può aggiungere degli attributi certificati a una versione di un e-service template (Parte2#Scenario intorno a 197)
    ...
    And l'utente crea un gruppo di attributi associati all'e-service template contenente 1 attributo certificato con successo
    ...
    When l'utente tenta di aggiungere gli attributi certificati numeri da 2 a 3 al gruppo dell'e-service template
    ...
    And [si prende nota dello stato degli attributi certificati del gruppo dell'e-service template]
    And gli attributi certificati sono stati aggiunti correttamente al gruppo dell'e-service template
    And i precedenti attributi certificati del gruppo dell'e-service template sono rimasti invariati
    When l'utente tenta di aggiungere l'attributo certificato numero 4 al gruppo dell'e-service template
    Then si ottiene http status code 204
  ...
  @m2m-parte2-ottobre
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_ADD_03] Un utente con ruolo M2M-ADMIN NON può aggiungere degli attributi certificati a una versione di un e-service template che non gli appartiene (Parte2#Scenario intorno a 115)
    Given l'utente è un "admin" di "PA1"
    And l'utente effettua la creazione di un e-service template in modalità <mode> in stato di DRAFT
    ...
    When l'utente tenta di aggiungere l'attributo certificato numero 2 al gruppo dell'e-service template
    Then si ottiene lo status code 403
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
    And [si prende nota dello stato degli attributi certificati del gruppo dell'e-service template]
    And gli attributi certificati del gruppo dell'e-service template sono rimasti invariati
    Examples:
      | mode        |
      | erogazione  |
  ...
  @m2m-parte2-ottobre
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_ADD_04] Un utente con ruolo M2M-ADMIN NON può aggiungere degli attributi certificati a una versione di un e-service template indicando degli identificativi inesistenti (Parte2#Scenario intorno a 119)
    Given l'utente è un "admin" di "PA1"
    And l'utente effettua la creazione di un e-service template in modalità <mode> in stato di DRAFT
    And l'utente crea 2 attributi certificati con successo
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
    ...
    When l'utente tenta di aggiungere l'attributo certificato numero 2 al gruppo dell'e-service template indicando un e-service template id inesistente
    Then si ottiene lo status code 404
    And [si prende nota dello stato degli attributi certificati del gruppo dell'e-service template]
    And gli attributi certificati del gruppo dell'e-service template sono rimasti invariati

    When l'utente tenta di aggiungere l'attributo certificato numero 2 al gruppo dell'e-service template indicando un descriptor id inesistente
    Then si ottiene lo status code 404
    And [si prende nota dello stato degli attributi certificati del gruppo dell'e-service template]
    And gli attributi certificati del gruppo dell'e-service template sono rimasti invariati

    When l'utente tenta di aggiungere l'attributo certificato numero 2 al gruppo dell'e-service template indicando un group index inesistente
    Then si ottiene lo status code 404
    And [si prende nota dello stato degli attributi certificati del gruppo dell'e-service template]
    And gli attributi certificati del gruppo dell'e-service template sono rimasti invariati
  ...
  @m2m-parte2-ottobre
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_ADD_05] Un utente NON può aggiungere degli attributi certificati a una versione di un e-service template indicando un auth. token non valido (Parte2#Scenario intorno a 120)
    Given l'utente è un "admin" di "PA1"
    And l'utente effettua la creazione di un e-service template in modalità <mode> in stato di DRAFT
    And l'utente crea 2 attributi certificati con successo
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
  ...
  @m2m-parte2-ottobre
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_ADD_06] Un utente con ruolo M2M NON può aggiungere degli attributi certificati a una versione di un e-service template (Parte2#Scenario intorno a 121)
    Given l'utente è un "admin" di "PA1"
    And l'utente effettua la creazione di un e-service template in modalità <mode> in stato di DRAFT
    And l'utente crea 2 attributi certificati con successo
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
  ...
  @m2m-parte2-ottobre
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_ADD_07] Un utente con ruolo M2M-ADMIN NON può aggiungere degli attributi NON certificati a una versione di un e-service template (Parte2#Scenario intorno a 197)
    Given l'utente è un "admin" di "PA1"
    And l'utente effettua la creazione di un e-service template in modalità <mode> in stato di DRAFT
    And l'utente crea 1 attributi dichiarati con successo
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
    And l'utente crea un gruppo di attributi associati all'e-service template contenente 0 attributi certificati con successo
    And "PA1" porta la versione dell'e-service template in stato PUBLISHED
    And [si prende nota dello stato degli attributi certificati del gruppo dell'e-service template]
  ...
  @m2m-parte2-ottobre
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_LIST_01_B] Un utente con ruolo M2M o M2M-ADMIN può leggere gli attributi certificati di una versione di un e-service template in stato DRAFT solo se appartiene all'ente creatore (Parte2#Scenario intorno a 244)
    Given l'utente è un "admin" di "PA1"
    And l'utente effettua la creazione di un e-service template in modalità <mode> in stato di DRAFT
    And l'utente crea 2 attributi certificati con successo
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
  ...
  @m2m-parte2-ottobre
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_LIST_02] Un utente con ruolo M2M o M2M-ADMIN NON può leggere gli attributi certificati di una versione di un e-service template indicando degli identificativi inesistenti (Parte2#Scenario intorno a 247)
    Given l'utente è un "admin" di "PA1"
    And l'utente effettua la creazione di un e-service template in modalità <mode> in stato di DRAFT
  ...
  @m2m-parte2-ottobre
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_LIST_03] Un utente NON può leggere gli attributi certificati di una versione di un e-service template indicando un auth. token non valido (Parte2#Scenario intorno a 246)
    Given l'utente è un "admin" di "PA1"
    And l'utente effettua la creazione di un e-service template in modalità <mode> in stato di DRAFT
    And l'utente crea 1 attributi certificati con successo
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
  ...
  @m2m-parte2-ottobre
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_DELETE_02] Un utente con ruolo M2M NON può rimuovere gli attributi certificati di una versione di un e-service template (Parte2#Scenario intorno a 270)
    Given l'utente è un "admin" di "PA1"
    And l'utente effettua la creazione di un e-service template in modalità <mode> in stato di DRAFT
    And l'utente crea 2 attributi certificati con successo
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
  ...
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_DELETE_03] Un utente NON può rimuovere gli attributi certificati di una versione di un e-service template indicando degli identificativi inesistenti o appartenenti ad attributi già rimossi (Parte2#Scenario intorno a 271, 273)
    ...
    When l'utente tenta di rimuovere l'attributo certificato già eliminato numero 1 dal gruppo dell'e-service template
    Then si ottiene lo status code 404
    And gli attributi certificati del gruppo del template sono rimasti invariati
    Examples:
      | mode        |
  ...
  @m2m-parte2-ottobre
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_DELETE_04] Un utente NON può rimuovere gli attributi certificati di una versione di un e-service template indicando un auth. token non valido (Parte2#Scenario intorno a 272)
    Given l'utente è un "admin" di "PA1"
    And l'utente effettua la creazione di un e-service template in modalità <mode> in stato di DRAFT
    And l'utente crea 1 attributi certificati con successo
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
  ...
  @m2m-parte2-ottobre
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_DELETE_05] Un utente NON può rimuovere gli attributi certificati di una versione di un e-service template che non gli appartiene (Parte2#Scenario intorno a 274)
    Given l'utente è un "admin" di "PA1"
    And l'utente effettua la creazione di un e-service template in modalità <mode> in stato di DRAFT
    And l'utente crea 2 attributi certificati con successo
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
  ...
  Scenario Outline: [M2M_ES_TEMPLATES_CERTIFIED_ATTRIBUTES_DELETE_06] Un utente NON può rimuovere gli attributi certificati da una versione di un e-service template in stato diverso da DRAFT (Parte2#Scenario intorno a 275)
    ...
    And l'utente crea 1 attributi certificati con successo
    And l'utente è un "admin" di "PA1" con ruolo M2M m2m-admin
    And l'utente crea un gruppo di attributi associati all'e-service template contenente 1 attributi certificati con successo
    And "PA1" porta la versione dell'e-service template in stato <stato>
    When l'utente tenta di rimuovere l'attributo certificato numero 1 dal gruppo dell'e-service template
