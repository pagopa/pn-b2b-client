...
Feature: Attivazione e riattivazione di una finalità
  ...
  Scenario: Per una finalità precedentemente creata da un fruitore e attivata da un erogatore, la quale è stata successivamente portata in stato SUSPENDED, alla richiesta di riattivazione da parte di un utente con sufficienti permessi (admin) dell’ente che ha sospeso la finalità (erogatore), va a buon fine
  ...
  @sad-path
  @purpose_activation3
  Scenario: Per una finalità precedentemente creata da un fruitore e attivata da un erogatore, alla richiesta di riattivazione da parte di un utente con sufficienti permessi (admin), non va a buon fine, in questa casistica non è rispettata l'idempotenza.
    Given l'utente è un "admin" di "PA1"
    Given "PA2" ha già creato e pubblicato 1 e-service
    Given "PA1" ha una richiesta di fruizione in stato "ACTIVE" per quell'e-service
  ...
  Scenario: Per una finalità precedentemente creata da un fruitore e che è in stato REJECTED, alla richiesta di attivazione da parte di un utente con sufficienti permessi (admin) dell’ente erogatore, ottiene un errore
    Given l'utente è un "admin" di "PA2"
    Given "PA2" ha già creato e pubblicato 1 e-service
    Given "PA1" ha una richiesta di fruizione in stato "ACTIVE" per quell'e-service
    Given "PA1" ha già creato 1 finalità in stato "WAITING_FOR_APPROVAL" per quell'eservice
