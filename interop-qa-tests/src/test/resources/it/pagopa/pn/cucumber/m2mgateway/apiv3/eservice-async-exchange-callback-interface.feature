Feature: Gestione della callback interface per gli e-service asincroni
  ...
  Scenario: [ASYNC_ESERVICE_CALLBACK_INTERFACE_DELETE_ARCHIVED] La rimozione dell'interfaccia di callback per un e-service
  asincrono che si trova in stato ARCHIVED non può essere effettuato.
