...
/* Classe shadow della sua omonima in AWS SDK, usata per collocare log in punti strategici e
* altre misure di diagnostica per QA-7748. Temporanea, rimuovere una volta risolto.
* IN NESSUN CASO la soluzione del problema in oggetto deve basarsi su modifiche di questa classe. */
...
public final class ProfileCredentialsProvider
    ...
    private AwsCredentialsProvider createCredentialsProvider(ProfileFile profileFile, String profileName) {
        log.info("Loaded credentials from profile file: {} with profile name: {}", profileFile, profileName);
        // Load the profile and credentials provider
        AwsCredentialsProvider awsCredentialsProvider = profileFile.profile(profileName)
            .flatMap(p -> new ProfileCredentialsUtils(profileFile, p,
        ...
        log.info("Loaded credentials provider: {}", awsCredentialsProvider);
